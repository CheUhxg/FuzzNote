...
其次，我们通过减少fuzzing框架必须检查的系统调用数量来加快内核fuzzing。例如，在Linux系统4.10中，大约有291个系统调用。它们对应于Linux系统内核提供的不同服务。为了识别那些可以解引用悬浮指针的调用，一种直接的方法是对所有系统调用执行模糊测试。很明显，这将大大降低寻找对促进利用真正有用的系统调用的效率。
